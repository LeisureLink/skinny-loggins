...
function createFileTransport(config) {
  var settings = _.merge(defaults.file, config);
  settings.filename = path.resolve(settings.filename);
  logger.add(winston.transports.File, settings);
};

function createConsoleTransport(config) {
  var settings = _.merge(defaults.console, config);
